...
function tp_add_settings_tab( $settings_tabs ) {
	$settings_tabs['twoo_performant_uprise'] = __( '2 Performant', 'twoo-performant-uprise' );
...
function get_twoo_performant_uprise_settings() {
	...
	$settings = array(
		...
		'big_bear'        => array(
			...
			'type' => 'text',
			'desc' => 'You can find the value <a href="https://businessleague.2performant.com/advertiser/attribution/big_bear_attribution#section_1" target="_blank">here</a>; It is usually right after attr-2p.com/THIS_IS_THE_ID/clc/1.js',
			'id'   => 'tp_big_bear'
		),
		'section_end'     => array(
			'type' => 'sectionend',
...
function tp_enqueue_scripts() {
	if ( ! empty( get_option( 'tp_big_bear' ) ) ) {
		wp_enqueue_script( 'tp_big_bear', 'https://attr-2p.com/b' . get_option( 'tp_big_bear' ) . '/clc/1.js', array(), null, true );
	}
}
